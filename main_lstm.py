...
parser.add_argument('--lstm', type=str, default='lstm')
...
parser.add_argument('--seg-audio', action='store_true', default=False)
parser.add_argument('--lr', type=float, default=0.0005) 
parser.add_argument('--mono-coe', type=float, default=0.001)
parser.add_argument('--load-model', type=str, default='./assets/learned_models/1s-height-2layer/default_lstm2_h256_whole_bs128_bottle0to0_mono_coe0.001_345.model')    
parser.add_argument('--load-epoch', type=int, default=1) 
parser.add_argument('--model-path', type=str, default='./assets/learned_models/1s-height-2layer',
...
args.tag += "_{}{}_h{}_{}_bs{}_bottle{}to{}{}{}_mono_coe{}".format(args.lstm,
...
thresh_acc = [1,2,5,10]  # unit mm
snr_list = [-10,-8,-6,-4,-2,0,2,4,6,8,10,1000]
...
def compute_acc(height, target):
    ...
    # print('is_cor:', np.array(is_correct).shape)
...
def get_db_type(loader,db):
    audio_send = torch.tensor([]).cuda()
    target_send = torch.tensor([]).cuda()
    for batch_idx, (audio, target, db_type) in enumerate(loader):
        if args.cuda:
            audio, target, db_type = audio.cuda(), target.cuda(), db_type.cuda()
        # print(target.shape)
        audio = audio[db_type==db]
        target = target[db_type==db]
        # print(len(audio))
        # print('target_cut_shape:',target_send.shape)
        audio_send = torch.cat((audio_send,audio),0)
        target_send = torch.cat((target_send,target),0)
    print('{}db, audio_len:{},'.format(db, len(audio_send)))
    # print('target.shape:',target_send.shape)
    return audio_send, target_send
        
def test_db(model_, loader, db):
    model_.eval()
    ...
    test_loss_mse = 0
    res = []
    audio,target = get_db_type(loader, db) 
    batch_id = int(len(audio)/args.bs)
    if batch_id == 0:
        print('for db_type = {}, less than 128 samples, can not handle!'.format(db))
        return [0,0,0],db
    for id in range(1,batch_id+1):
        audio_batch = audio[args.bs*(id-1):args.bs*id]
        target_batch = target[args.bs*(id-1):args.bs*id]
    
        model_.hidden = model_.init_hidden(device)
        height, hidden = model_(audio_batch)
        height = height * (bottle_upper - bottle_lower) + bottle_lower
        
        target_batch = target_batch.view(-1, 1)
        loss_mse, loss_mono = loss_function(height, target_batch)
        test_loss_mse += loss_mse
        test_loss_mono += loss_mono

        res_acc = compute_acc(height, target_batch)
        correct_height = [c + r for c, r in zip(correct_height, res_acc)]
        data_num += target_batch.shape[0]

        # compute average height error
        test_error += torch_func.l1_loss(height, target_batch, size_average=False)
        res.append(target_batch)
...
def test(model_, loader):
    ...
    for batch_idx, (audio, target) in enumerate(loader):
        ...
        height = height * (bottle_upper - bottle_lower) + bottle_lower
        # print(height.shape)
        # print('all_test_target.shape:',target.shape)
    ...
    if args.mode == 'train':
        for epoch in range(is_resume * args.load_epoch, args.epoch):
            ...
            if epoch % args.save_interval == 0 and epoch != 0:
                path = os.path.join(args.model_path, args.tag + '_{}.model'.format(epoch+50))
        ...
        #     acc_db_test,db_int = test_db(model,test_loader,snr_list[i])
        #     print('Test for db={}: acc_test={}'.format(db_int, acc_db_test))
