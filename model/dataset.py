...
class PouringDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        tag = np.load(os.path.join(self.path, tmp_path[0], tmp_path[1]), allow_pickle=True)
        if not self.train_rnn:
            tag = np.squeeze(tag)
            target = np.array(tag[2]).astype(np.float32)
            # db_type = np.array(tag[4])
        else:
            tag = np.squeeze(tag)
            target = np.array(tag[2]).astype(np.float32) #tag[2]直接对应高度mm，tag[3]对应重量g
            # db_type = np.array(tag[4])
...
if __name__ == "__main__":
    ...
    b = PouringDataset("../dataset/", input_audio_size=257, multi_modal=False,
                       train_rnn=True, is_fixed=False, is_train=True, seg_audio=False)
