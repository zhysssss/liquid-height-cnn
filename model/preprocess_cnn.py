...
config_file = os.path.join(pkg_path, "config/preprocess_cnn.yaml")
config = yaml.load(open(config_file, "r"))
fixed_length = config["fixed_audio_length"]  # unit seconds
assert fixed_length == 4.09, "check fixed length"
...
assert win_length == 512 , "win_length should larger equal than n_fft"
...
def read_talk_noise(path, array_list):
    file_list = os.listdir(path)
    for file in file_list:
        talk, talk_sr = librosa.load(path+'/'+file, sr=44100)
        talk_resample = librosa.core.resample(talk, orig_sr=talk_sr, target_sr=8000)
        array_list.append(talk_resample)
    return array_list

talk_array_list = []
talk_list = read_talk_noise('/content/drive/MyDrive/AudioPouring-master/audio_pouring/model/other-noise/people',talk_array_list)
bgm_array_list = []
bgm_list = read_talk_noise('/content/drive/MyDrive/AudioPouring-master/audio_pouring/model/other-noise/bgm',bgm_array_list)
env_array_list = []
env_list = read_talk_noise('/content/drive/MyDrive/AudioPouring-master/audio_pouring/model/other-noise/env',env_array_list)
...
def main(pickle_path, npy_path, multi_threading):
    ...
    if multi_threading:
        ...
        def task(task_inter_, i_, pickle_files_, npy_path_):
            ...
            data_ = pickle.load(open(pickle_file_, "rb"), encoding='iso-8859-1')
        ...
        for i in range(pickle_num):
            with (open(pickle_files[i], "rb")) as openfile:
                data = pickle.load(openfile, encoding='iso-8859-1')
...
def npy_generate(data, pickle_file, npy_path):
    ...
    for ind in range(len(start_index)):
        s_index = int(start_index[ind])
        e_index = int(end_index[ind])
        cavity_h = np.array([])
        whole_scale = np.array([])

        audio_spec,db = audio_process(audio_raw[s_index: e_index], orig_sr, target_frequency, vis_audio,
        ...
        np.save(npy_name, [np.array([name]), audio_spec, cavity_h, whole_scale_relative, db])
...
def signal_by_db(x1, x2, snr): #x1:clean sound    x2: added noise
...
def add_talk(data, talk, snr):
    length = len(data)
    max_start = len(talk) - length
    li = [i for i in range(0, max_start)]
    start = np.random.choice(li, 1, replace=False)
    start = start[0]
    talk_use = talk[start:start+length]
    after_add = signal_by_db(data, talk_use, snr)
    return after_add
    

def add_noise(src, SNR):
...
def audio_process(audio, orig_sr, target_frequency_, vis, fig_name):
    ...
    if add_noise:
        ...
        if choice == 1: # no change
            audio_resample = audio_resample
            db = 1000
            
        elif choice == 4: # add white noise
            snr_list = [-10,-8,-6,-4,-2,0,-10,-8,-6,-4,-2,0,2,4,6,8,10]
            snr_db = np.random.choice(snr_list, 1)[0]
            audio_resample = add_noise(audio_resample, snr_db)
            db = snr_db
            
        elif choice == 2: # add talk noise
            c_list = [i for i in range(len(talk_list))]
            c_choice = np.random.choice(c_list, 1)[0]
            snr_list = [-10,-8,-6,-4,-2,0,-10,-8,-6,-4,-2,0,2,4,6,8,10]
            snr_db = np.random.choice(snr_list, 1)[0]
            audio_resample = add_talk(audio_resample, talk_list[c_choice],snr_db)
            db = snr_db
            
        elif choice == 3:  #add bgm noise
            b_list = [i for i in range(len(bgm_list))]
            b_choice = np.random.choice(b_list, 1)[0]
            snr_list = [-10,-8,-6,-4,-2,0,-10,-8,-6,-4,-2,0,2,4,6,8,10]
            snr_db = np.random.choice(snr_list, 1)[0]
            audio_resample = add_talk(audio_resample, bgm_list[b_choice],snr_db) 
            db = snr_db
            
        else: #add env noise
            e_list = [i for i in range(len(env_list))]
            e_choice = np.random.choice(e_list, 1)[0]
            snr_list = [-10,-8,-6,-4,-2,0,-10,-8,-6,-4,-2,0,2,4,6,8,10]
            snr_db = np.random.choice(snr_list, 1)[0]
            audio_resample = add_talk(audio_resample, env_list[e_choice],snr_db) 
            db = snr_db  
    ...
    audio_re_fft = audio_re_fft[:128, :] #取前128个频率段
...
if __name__ == "__main__":
    ...
    # print('num_no:{}, num_white:{}, num_talk:{}, num_bgm:{}, num_env:{}'.format(num_no, num_white, num_talk, num_bgm, num_env))
