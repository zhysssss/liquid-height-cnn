...
parser.add_argument('--bottle', type=str, default='0')
parser.add_argument('--lr', type=float, default=0.001)
parser.add_argument('--load-model', type=str, default='./assets/learned_models/cnn-8k-128out-noise/default_300.model')
parser.add_argument('--load-epoch', type=int, default=1)
parser.add_argument('--model-path', type=str, default='./assets/learned_models/cnn-8k-128out-noise',
...
if is_resume or args.mode == 'test':
    ...
    model = AudioCNN(input_size=input_audio_size)
...
def train(model_, loader, epoch):
    ...
    data_num = 0
    for batch_idx, (audio, target,_) in enumerate(loader):
        if args.cuda:
...
def main():
    if args.mode == 'train':
        ...
        #     acc_db_test = test_db(model,test_loader,snr_list[i])
        #     print('Test for db={}: acc_test={}'.format(snr_list[i], acc_db_test))
